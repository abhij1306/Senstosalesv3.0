...
class ReconciliationServiceV2:
    ...
    def sync_po(db: sqlite3.Connection, po_number: str):
        ...
        try:
            ...
            db.executemany("UPDATE purchase_order_items SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?", item_status_updates)
            ...
            # 5. Update PO Status
            db.execute("UPDATE purchase_orders SET po_status = ?, updated_at = CURRENT_TIMESTAMP WHERE po_number = ?", (po_status, po_number))
